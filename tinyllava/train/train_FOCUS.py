...
from Trainer import ORPOTrainer, SimPOTrainer
...
class LLaVADPODataCollator(DPODataCollatorWithPadding):
    ...
    def tokenize_batch_element(
        ...
        if "img" in prompt:
            ...
            chosen_conv, rejected_conv = preprocess_multimodal(
                [copy.deepcopy(chosen_conv), copy.deepcopy(rejected_conv)], data_args=self.data_args)
            # self.data_args None)
        # tokenize
...
def bpo_paired_dataset(ds, local_rank, data_args, num_ite=1):
    ...
    def make_batch_pairs(sample):
        ...
        for sample_idx, (prompt, image, completions) in enumerate(zip(sample["prompt"], sample["image"], sample["completions"])):
            ...
            rejected = completions[num_ite]["response"]
...
class CustomDPOTrainer(ORPOTrainer):
    def odds_ratio_loss(
            self,
        ...
        # )
        log_odds_w = (policy_chosen_logps - torch.log1p(-torch.exp(policy_chosen_logps)) )
        odds_l = (torch.exp(policy_rejected_logps)+1e-12) / ((1-torch.exp(policy_rejected_logps))+1e-12 )
        d_clip = torch.clamp(input=odds_l, min=0, max=threshold)
        # print(log_odds_w, odds_l, d_clip)
        log_odds = log_odds_w - torch.log(odds_l/d_clip)


        # sig_ratio = F.sigmoid(log_odds)
        # ratio = torch.log(sig_ratio+1e-12)
        ratio = F.logsigmoid(log_odds-gamma)
    ...
    def _get_batch_logps(
        ...
        if weighted and len_chosen > 0:
            chosen_ids = labels[:len_chosen]
            reject_ids = labels[len_chosen:]
            win_token_weight = torch.ones_like(per_token_logps[:len_chosen])
            rej_token_weight = torch.ones_like(per_token_logps[len_chosen:])
            for idx, (w, r) in enumerate(zip(chosen_ids, reject_ids)):
                valid_w = w[1:]
                valid_r = r[1:]
                ...
                win_token_weight[idx][w_mod] = 3.0
                rej_token_weight[idx][r_mod] = 3.0
            token_weight = concate_pad(win_token_weight, rej_token_weight, 0)
            loss_mask = token_weight * loss_mask
    ...
    def concatenated_forward(
        ...
        chosen_nll_loss = cross_entropy_loss(all_logits[:len_chosen], all_labels[:len_chosen])
...
def train():
    ...
    if model_args.vision_tower is not None:
        if model_args.lora_path is not None:
            lora_cfg_pretrained = AutoConfig.from_pretrained(model_args.lora_path)
            ...
            if os.path.exists(os.path.join(model_args.lora_path, 'non_lora_trainables.bin')):
                non_lora_trainables = torch.load(os.path.join(model_args.lora_path, 'non_lora_trainables.bin'),
                                                 map_location='cpu')
            else:
                ...
                non_lora_trainables = load_from_hf(model_args.lora_path, 'non_lora_trainables.bin')
    ...
    for i in range(data_args.num_ite):
        print(i+1,"    Start!")
        train_dataset = bpo_paired_dataset(dataset, local_rank, data_args,num_ite = i+1)

        print(f"rank {local_rank} train length {len(train_dataset)}")
        training_args.output_dir = output_dir+f"--{i+1}"
