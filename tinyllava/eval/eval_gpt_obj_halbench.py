    ...
    try:
        ...
        if '[' in content:
            try:
                ...
                raise ValueError('Content is not json interpretable1')
        else:
            raise ValueError('Content is not json interpretable2')
    return content
...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path, openai_apikey):
        ...
        self.chat_model = Chat(model="gpt-3.5-turbo", timeout_sec=100, openai_apikey=openai_apikey)
        self.fail_limit=100


        #read in synonyms
        synonyms = open('./playground/data/eval/obj_halbench/synonyms_refine.txt').readlines()
